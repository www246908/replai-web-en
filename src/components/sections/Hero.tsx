...
// ── Magnetic button — moves toward cursor (keeps shadow-only hover so the
//    inline transform doesn't fight Tailwind's translate utilities) ──
    ...
    <div className="relative mx-auto w-full max-w-[340px]">
      ...
      <div className="relative overflow-hidden rounded-[2.25rem] border-2 border-[var(--pg-fg)] bg-white shadow-[8px_8px_0_0_var(--pg-fg)]">
        ...
        <div className="bg-pg-accent px-5 pb-4 pt-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="font-pg-display text-sm font-bold leading-none text-white">Replai Assistant</p>
                <p className="mt-0.5 font-pg-body text-sm text-white/70">Auto-replying now</p>
              </div>
            </div>
            <div className="flex items-center gap-1">
              <span className="h-2 w-2 animate-pulse rounded-full bg-pg-tertiary" />
              <span className="font-pg-body text-sm text-white/70">Live</span>
            </div>
          </div>
        </div>

        {/* Messages */}
        <div className="flex min-h-[300px] flex-col gap-2.5 bg-pg-muted px-3 py-4">
          {CHAT_SEQUENCE.map((msg) => {
            ...
            return (
              ...
              >
                <div className={`max-w-[80%] border-2 border-[var(--pg-fg)] px-3.5 py-2.5 font-pg-body text-sm leading-relaxed ${isAI ? "rounded-2xl rounded-bl-sm bg-white text-pg-fg" : "rounded-2xl rounded-br-sm bg-pg-accent text-white"}`}>
                  {msg.text}
                </div>
              </motion.div>
            );
        ...
        <div className="flex items-center gap-2 border-t-2 border-[var(--pg-fg)] bg-white px-3 py-2.5">
          <div className="flex-1 rounded-full bg-pg-muted px-4 py-2 font-pg-body text-sm text-pg-muted-fg">Type a message...</div>
      ...
      <motion.div
        ...
        className="absolute -right-3 -top-3 rotate-3 rounded-full border-2 border-[var(--pg-fg)] bg-pg-secondary px-3 py-1.5 font-pg-display text-xs font-bold text-white shadow-[3px_3px_0_0_var(--pg-fg)]"
      >
        Auto Reply
      </motion.div>
      <motion.div
        ...
        className="absolute -bottom-3 -left-3 flex -rotate-2 items-center gap-1.5 rounded-full border-2 border-[var(--pg-fg)] bg-white px-3 py-1.5 font-pg-body text-sm font-medium text-pg-fg shadow-[3px_3px_0_0_var(--pg-fg)]"
      >
...
export default function Hero() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative flex min-h-screen items-center overflow-hidden bg-pg-bg pb-16 pt-24"
    >
      {/* Dot-grid paper texture */}
      <div aria-hidden className="pg-dots pointer-events-none absolute inset-0 opacity-50" />

      {/* Big confetti shapes (hidden on mobile so they never overlap copy) */}
      <div aria-hidden className="pointer-events-none absolute inset-0 hidden overflow-hidden lg:block">
        {/* massive tertiary circle behind the headline */}
      ...
      <div className="relative mx-auto w-full max-w-6xl px-5 sm:px-8">
        <div className="grid items-center gap-14 lg:grid-cols-2 lg:gap-12">
          ...
          {/* ── Left: copy 桌面版整塊往左移 lg:-ml-6 xl:-ml-12 ── */}
          <div className="order-2 lg:order-1 lg:-ml-6 xl:-ml-12">
            {/* Badge 同步左移 */}
            <motion.div {...reveal(0)} animate={isInView ? { opacity: 1, y: 0, filter: "blur(0px)" } : {}}>
              <span className="mb-7 inline-flex items-center gap-2 rounded-full border-2 border-[var(--pg-fg)] bg-pg-tertiary px-4 py-1.5 font-pg-display text-sm font-bold text-pg-fg shadow-[3px_3px_0_0_var(--pg-fg)] lg:-ml-6 xl:-ml-12">
                <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-pg-secondary" />
                AI Customer Service, Open 24/7
              </span>
            </motion.div>

            {/* Headline — 禁止拆字、加大行高、波浪線上移不遮字 */}
            <h1 
              className="mb-3 font-pg-display text-5xl font-extrabold leading-[1.2] tracking-tight text-pg-fg sm:text-6xl lg:text-[3.75rem] xl:text-7xl"
            ...
            <motion.p
              ...
              className="mb-8 mt-6 max-w-md font-pg-body text-lg leading-relaxed text-pg-muted-fg sm:text-xl"
            >
            ...
            >
              {/* Primary — magnetic candy button */}
              <MagneticLink
                href="/contact"
                className="group inline-flex items-center justify-center gap-3 rounded-full border-2 border-[var(--pg-fg)] bg-pg-accent px-7 py-3.5 font-pg-display text-base font-bold text-white shadow-[4px_4px_0_0_var(--pg-fg)] transition-shadow duration-300 hover:shadow-[6px_6px_0_0_var(--pg-fg)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-pg-accent/40"
              >
              ...
              {/* Secondary — outline fills yellow on hover */}
              <MagneticLink
                href="/#how-it-works"
                className="inline-flex items-center justify-center rounded-full border-2 border-[var(--pg-fg)] bg-transparent px-7 py-3.5 font-pg-display text-base font-bold text-pg-fg transition-colors duration-200 hover:bg-pg-tertiary focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-pg-accent/40"
              >
                See How It Works
              </MagneticLink>
            </motion.div>

            {/* Social proof 小字放大 text-sm */}
            <motion.div
              {...reveal(0.82)}
              animate={isInView ? { opacity: 1, y: 0, filter: "blur(0px)" } : {}}
            ...
            >
              <div className="flex -space-x-2">
                {[
                  ...
                  <div key={i} className={`flex h-10 w-10 items-center justify-center rounded-full border-2 border-[var(--pg-fg)] ${item.bg} font-pg-display text-[10px] font-bold ${item.label === "Fitness" ? "text-pg-fg" : "text-white"}`}>
                    {item.label}
                  </div>
                ))}
              </div>
          ...
          {/* ── Right: phone 桌面版往右偏移 lg:-mr-8 xl:-mr-16 ── */}
          <motion.div
            ...
            className="order-1 flex justify-center lg:order-2 lg:-mr-8 xl:-mr-50"
          >
